...
class VrepArm(gym.Env):
    ...
    def __init__(self, simulation=False):
        ...
        _, self.armHandle = vrep.simxGetObjectHandle(self.clientID, 'PhantomXPincher', vrep.simx_opmode_blocking)
        _, self.objectHandle = vrep.simxGetObjectHandle(self.clientID, 'PhantomXPincher_target', vrep.simx_opmode_blocking)
        _, self.tip = vrep.simxGetObjectHandle(self.clientID, 'PhantomXPincher_tip', vrep.simx_opmode_blocking)

        joint_names = ["PhantomXPincher_joint1", "PhantomXPincher_joint2", "PhantomXPincher_joint3",
                       "PhantomXPincher_joint4", "PhantomXPincher_joint5"]
        self.joint_handles = []
        ...
        self.joint_restrictions = np.array([[-170.0, 170.0], [-135.0, 135.0], [-135.0, 135.0],
    ...
    def reset(self):
        # reset arm
        vrep.simxSetObjectPosition(self.clientID, self.armHandle, -1, (0, 0, 0.042200), vrep.simx_opmode_oneshot)
