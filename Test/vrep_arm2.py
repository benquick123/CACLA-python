...
class VrepArm(gym.Env):
    ...
    def __init__(self, action_multiplier=1.0, simulation=False):
        ...
        low = np.hstack((0.0, np.array([-np.inf] * 3), low))
        high = np.hstack((np.inf, np.array([np.inf] * 3), high))
    ...
    def step(self, action, absolute=False):
        ...
        done = True if distance < 0.01 or self.iteration_n > 25 else False

        info = {"distance": distance, "actual_action": actual_action}

        return observation, reward, done, info

    def phantom_step(self, action, absolute=False):
        action = np.array(action)
        action[action > 1.0] = 1.0
        action[action < -1.0] = -1.0
